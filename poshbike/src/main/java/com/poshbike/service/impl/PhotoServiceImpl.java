...
import com.poshbike.PhotoInfo;
import com.poshbike.service.PhotoService;
...
public class PhotoServiceImpl implements PhotoService {
	...
	private PhotoInfo createPhoto(String id, String userId) {
		...
		photo.setResourceURL("/com/poshbike/impl/resources/"
				+ photo.getName());
		return photo;
	}
	...
	public InputStream loadPhoto(String id) {
		...
		if (authentication.getPrincipal() instanceof UserDetails) {
			...
			for (PhotoInfo photoInfo : getPhotos()) {
				if (id.equals(photoInfo.getId())
						&& username.equals(photoInfo.getUserId())) {
					URL resourceURL = getClass().getResource(
							photoInfo.getResourceURL());
					if (resourceURL != null) {
						try {
